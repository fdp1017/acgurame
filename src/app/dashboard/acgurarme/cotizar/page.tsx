...
export default function Cotizar() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        const opcionesRef = collection(db as any, 'opcionesSeguro');
  ...
  const verificarCodigoPromo = async (codigo: string) => {
    try {
      const codigosRef = collection(db as any, 'codigosPromo');
  ...
  const activarSeguro = async () => {
    try {
      ...
      // Guardar la póliza
      const seguroDocRef = doc(db as any, 'seguros', `${user.uid}_${Date.now()}`);
